...
int main()
{
  ...
  char *token_array[MAXIMAL_NUMER_OF_TOKENS];
  command commands[NUMBER_OF_COMMANDS] = {
      {CONNECT, "/connect", 1, OFFLINE, connect_callback},
      {NICK, "/nick", 1, ONLINE, change_nickname},
  };
  fd_set readfd;
  ...
  while (1)
  {
    ...
    else if (FD_ISSET(STDIN_FILENO, &readfd))
    {
      ...
      if ((buf_len = read(STDIN_FILENO, msg, sizeof(msg))))
      {
        ...
        msg[buf_len + 2] = '\0';
        printf("debug2");
        if (is_first_char_slash(msg))
        {
          int number_of_tokens = tokenize(msg, token_array);
          printf("debug1");
          for (int i = 0; i < NUMBER_OF_COMMANDS; i++)
          {
            if ((strcmp(commands[i].identifier, token_array[0]) == 0) && (number_of_tokens >= commands[i].token_count)) // and status matches
            {
              commands[i].callback(NULL, token_array, &number_of_tokens);
            }

            //daca token count is >= command config token count.
            //apelez callback
          }
        }
        else
